...
#define CELL(g, y, x) g[(y) * stride + (x)] // use stride for padded grid

uint8_t *allocate_grid(int n, int stride) {
    return (uint8_t *)calloc(stride * stride, sizeof(uint8_t)); // zero init for halo
}
...
// v2 eliminates bounds checks, uses padded grid
static inline int count_neighbors(uint8_t *grid, int n, int stride, int x, int y) {
    int neighbors = 0;

    neighbors += CELL(grid, y - 1, x - 1) + CELL(grid, y - 1, x) + CELL(grid, y - 1, x + 1);
    neighbors += CELL(grid, y, x - 1) + CELL(grid, y, x + 1);
    neighbors += CELL(grid, y + 1, x - 1) + CELL(grid, y + 1, x) + CELL(grid, y + 1, x + 1);
